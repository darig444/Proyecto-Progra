...
function finalizarCompraa() {
    const loggedInUser = localStorage.getItem('loggedInUser'); //Obtener el usuario que esta logueado
    if (carrito.length === 0) {
    ...
    if (compraValida) {
        let comprasRealizadas = JSON.parse(localStorage.getItem('coomprasRealizadas')) || [];
        let nuevaaCompra = {
